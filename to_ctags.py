...
# Add support for RFC documents by tanhuacheng <tanhc.gz@gmail.com>
...
def findSectionsMarkdown(filename, lines):
    sections = []
    inCodeBlock = False
    inFrontMatter = False
...
def findSections(filetype, filename, lines):
    filetype = filetype.lower()

    if filetype == 'markdown' or filetype == 'md':
        return findSectionsMarkdown(filename, lines)
    elif filetype == 'rfc':
        return findSectionsRFC(filename, lines)
    else:
        return None
...
def main():
    ...
    parser.add_option(
        ...
        help='Write tags into FILE (default: "tags").  Use "-" to write '
             'tags to stdout.')
    parser.add_option(
        "-t", "--type", metavar="TYPE", dest='filetype',
        default="markdown",
        help='Indicate the filetype (default: "markdown") of input file(s).')
    parser.add_option(
    ...
    for filename in args:
        ...
        f.close()
        sections = findSections(options.filetype, filename, lines)
